...
|---|---|---|
| per-row `qk` (not per-head) | ✅ (PR #7) | ✅ (already) |
| `rep_g = R_g/n_g` subsample repricing | ✅ | ✅ (already) |
| **FLOP / `macs_per_row` weighting** | ✅ (PR #7) | ❌ **missing** |
| `W_g` per-group importance weights | ❌ | ✅ (`group_weights`) |
| `grad` / `measured` / `act_global` methods | ❌ | ✅ |
| MoE empty-expert handling | ❌ | ✅ |
| `PendingMerger` (grad, keyed by call id) | ❌ | ✅ |
