#!/bin/bash
# ============================================================
# 3-GPU parallel stoc_len sweep: ONE GPU per stoc_len.
#
#   GPU 0 -> uniform256, GPU 1 -> uniform192, GPU 2 -> uniform128.
#   Each GPU generates the full balanced NUM_FID set (10/class x 1000) for
#   its stoc_len, concurrently. sc_prec=8, halve ON (sweepable: explicit
#   stoc_len, rng grid -> 128), bitrev scramble, qk & av = per_row, all
#   operators + all timesteps SC. Resumable (idempotent on existing PNGs).
#
# Usage (from a >=3-GPU allocation):
#   conda activate qdit
#   cd .../scmp_diffusion
#   bash scripts/run_3gpu_parallel_stoc.sh
#
